...
namespace VNPAY_CS_ASPX
{
    public partial class _Default : System.Web.UI.Page
    {
        ...
        { //Get Config Info
            ...
            //Save order to db
            var orderId = HttpUtility.ParseQueryString(Request.Url.Query).Get("OrderId");
            var amount = HttpUtility.ParseQueryString(Request.Url.Query).Get("Amount");
            try
            {
                order.OrderId = long.Parse(orderId);
                order.Amount = long.Parse(amount);
            }
            catch
            {
                return;
